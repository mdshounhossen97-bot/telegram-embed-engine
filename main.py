...
async def startup_event():
    ...
    logging.info("Bot started. Attempting to find channel...")
    
    # documentation-er advice onujayi: dialogs check kora entity find korar jonno
    async for dialog in client.iter_dialogs():
        if dialog.id == CHANNEL_ID:
            logging.info(f"Successfully found and cached: {dialog.title}")
            break
...
async def embed_player(tmdb_id: str, s: int = Query(None), e: int = Query(None)):
    ...
    try:
        ...
        return f"<html><body style='background:#00;color:red;'><h1>Entity Error: {str(e)}</h1></body></html>"
